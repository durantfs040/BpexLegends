...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		protected override void Update()
		{
			...
			if (holdingButtonFire)
			{
				...
				if (CanPlayAnimationFire() && equippedWeapon.HasAmmunition() && equippedWeapon.IsAutomatic())
				{
					...
					{
						if(ammunitionCurrent.GetComponent<TextMeshProUGUI>().text != "0") Fire();
						ammunitionCurrent.GetComponent<TextMeshProUGUI>().text = (int.Parse(ammunitionCurrent.GetComponent<TextMeshProUGUI>().text) - 1).ToString() ;
					}
				}	
			}
        ...
        private bool CanPlayAnimationReload()
		{
			...
			if (inspecting)
				return false;
			if (ammunitionTotal.GetComponent<TextMeshProUGUI>().text != "0")
			{
				return true;
			}
			//Return.(##**temporarily false**##)
			return false;
		}
		...
		public void OnTryPlayReload(InputAction.CallbackContext context)
		{
			...
			switch (context)
			{
				...
				case {phase: InputActionPhase.Performed}:
					//Play Animation.
					if (int.Parse(ammunitionCurrent.GetComponent<TextMeshProUGUI>().text) == 30)
					{
						return;
					}
					PlayReloadAnimation();
		...
		private IEnumerator WaitForReload()
		{
			...
			// Update Ammo
			int currentAmmo = int.Parse(ammunitionCurrent.GetComponent<TextMeshProUGUI>().text);
			int totalAmmo = int.Parse(ammunitionTotal.GetComponent<TextMeshProUGUI>().text);
			int ammoNeeded = 30 - currentAmmo;
			int ammoToDeduct = Mathf.Min(ammoNeeded, totalAmmo);

			ammunitionCurrent.GetComponent<TextMeshProUGUI>().text = (currentAmmo + ammoToDeduct).ToString();
			ammunitionTotal.GetComponent<TextMeshProUGUI>().text = (totalAmmo - ammoToDeduct).ToString();
